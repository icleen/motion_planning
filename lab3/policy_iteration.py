...
def policy_iteration(map, discount=0.8, threshold=0.1, max_iterations=100):
    policy = defaultdict(lambda: 'u')
    values = defaultdict(lambda: 0)
    iters = 0
    changes = 1
    for iters in range(max_iterations):
        values, viters = policy_evaluation(map, policy, values, discount, threshold, max_iterations)
        policy, changes = policy_improvement(map, policy, values, discount)
        iters += 1
        if iters%10 == 0:
...
def policy_evaluation(map, policy, values, discount=0.8, threshold=0.1, max_iterations=100):
...
def policy_improvement(map, policy, values, discount=0.8):
    newpolicy = {}
    changes = 0
    for state in map.get_states():
        best_aval = float("-inf")
